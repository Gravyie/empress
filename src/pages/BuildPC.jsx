...
export default function BuildPC() {
  ...
  const total = useMemo(() => {
    ...
    Object.entries(sel).forEach(([key, value]) => {
      ...
      if (key === "processor") {
        ...
        item = (DATA[key] || []).find(i => i.label === value || i.id === value);
      }
      if (item) {
  ...
  // Get current PC image
  const currentImage = useMemo(() => {
    if (sel.case) {
      return PC_IMAGES[sel.case] || PC_IMAGES.default;
    }
  ...
  const downloadQuote = () => {
    ...
    Object.entries(sel).forEach(([k, v]) => {
      if (v) {
        ...
        text += `${k.charAt(0).toUpperCase() + k.slice(1)}: ${v}${count > 1 ? ` x${count}` : ''} - ₹${price}\n`;
      }
    });
    text += `\nTotal: ₹${total}`;
  ...
  const componentFields = [
    ...
    { label: "CASE", key: "case", options: DATA.cases },
  ...
  return (
    <div className="min-h-90vh bg-white text-gray-500 flex p-6">
      ...
      {/* Right Panel */}
      <div className="w-1/2 bg-gray-100 flex flex-col rounded-2xl">
        {/* Header */}
        <div className="p-6 border-b border-gray-700">
          {/* First Row - Components/Add-ons */}
            ...
            <label className="flex items-center cursor-pointer">
              <div className="relative">
                ...
                <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${
                  brand === "amd" ? "border-white bg-white" : "border-gray-400"
                }`}>
                  {brand === "amd" && <div className="w-3 h-3 bg-[#F47C5A] rounded-full"></div>}
                </div>
              </div>
              <span className="ml-3 text-gray-600 font-bold text-lg">AMD</span>
            </label>
            <label className="flex items-center cursor-pointer">
              <div className="relative">
                <input
                ...
                />
                <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${
                  brand === "intel" ? "border-white bg-white" : "border-gray-400"
                }`}>
                  {brand === "intel" && <div className="w-3 h-3 bg-[#F47C5A] rounded-full"></div>}
                </div>
              </div>
              <span className="ml-3 text-gray-600 font-bold text-lg">intel</span>
            </label>
          </div>
        </div>
                ...
                <div className={field.qty ? "flex-1" : "w-full"}>
                  ...
                  >
                    ...
                    {field.options.map((option) => (
                      <option key={option.label || option.id} value={option.label || option.id}>
                ...
                {field.qty && sel[field.key] && (
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setQty(prev => ({ ...prev, [field.key]: parseInt(e.target.value) }))}
                    >
                      {[1, 2, 3, 4].map(num => (
        ...
        <div className="p-6 border-t border-gray-700">
          <div className="flex justify-end space-x-4">
            <button
              ...
              className="bg-[#F47C5A] hover:bg-[#F47C5A] text-gray-700 font-semibold px-6 py-2 rounded transition-colors"
            >
              Download Quote
            </button>
            {phase === "components" ? (
              ...
              <button className="bg-[#F47C5A] hover:bg-[#F47C5A] text-gray-700 font-semibold px-6 py-2 rounded transition-colors">
                Add to Cart
              </button>
            )}
          </div>
